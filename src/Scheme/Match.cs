...
public abstract class Match
{
    ...
    public static Match<Tuple<SchemePair, TCar, TCdr>> Pair<TCar, TCdr>(Match<TCar> car, Match<TCdr> cdr)
    {
        return new($"pair of {car.Name}, {cdr.Name}", (SchemeObject input) =>
            input.TryTo<SchemePair>(out var p, out _) &&
                car.Func(p.Car) is Match<TCar>.Success cars &&
                cdr.Func(p.Cdr) is Match<TCdr>.Success cdrs
                ? new(p, cars.Value, cdrs.Value) : null);
    }
    ...
    public static readonly Match<SchemeSyntaxObject> Identifier = Syntax(AnySymbol).Map(x => x.Item1);

    public static readonly Match<SchemeSyntaxObject> Application = Syntax(Pair(Identifier, AnyDatum)).Map(x => x.Item1);
}
...
public sealed class Match<T> : Match
{
    ...
    public Match<U> Map<U>(Func<T, U> f)
    {
        return new Match<U>(Name, (o) =>
        {
            var x = Func(o);
            if (x is Success success)
                return new Match<U>.Success(f(success.Value));
            else
                return new Match<U>.Failure(((Failure)x).Message);
        });
    }
